...
export default defineComponent({
    ...
    setup() {
        const breadcrumbs: BreadcrumbItem[] = [{ title: 'Intelligence', href: '/mapping' }, { title: 'Map', href: '/mapping' }];
        ...
        const renderHeatmap = () => {
            ...
            if (!isHeatmapEnabled.value) {
                applyFilters(); // Restore normal view
                return;
            }

            // Dim the polygon layer to make heat visible
            allGeoJsonLayers.forEach(item => item.layer.setStyle({ fillOpacity: 0.05, opacity: 0.1 }));
        ...
        const applyFilters = () => {
            ...
            allGeoJsonLayers.forEach(item => {
                ...
                if (matchProv && matchCity) {
                    item.layer.setStyle({ opacity: 1, fillOpacity: 0.2 });
        ...
        const updateLabels = () => {
            // 1. Guard clause for initialization
            if (!labelLayer || !map) return;
            
            labelLayer.clearLayers();
            
            const zoom = map.getZoom();
            // 2. High zoom threshold to prevent performance lag/clutter
            if (zoom < 13) return;
            ...
            allGeoJsonLayers
                .filter(item => {
                    // 3. Safety Check: Ensure item and bounds exist and are valid
                    ...
                    // 5. Check Active Filters (Province AND City)
                    const matchProv = !filters.province || item.props.ADM2_EN === filters.province;
                    const matchCity = !filters.city || item.props.ADM3_EN === filters.city;
                    
                    return isVisible && matchProv && matchCity;
                })
                .slice(0, 50) // Limit count for performance
                .forEach(item => {
                    try {
                        // 6. Safe Center Acquisition
                        const center = item.bounds.getCenter();
                        
                        L.marker(center, {
                            icon: L.divIcon({
                                className: 'map-label', // Ensure this class is defined in your CSS
                                html: `<span>${item.props.ADM4_EN || 'Unknown'}</span>`,
                                iconSize: [100, 20],
                                iconAnchor: [50, 10] // Anchors the label to its center point
                            }),
                            interactive: false
                        }).addTo(labelLayer);
                    } catch (e) {
                        console.error("Label Error for:", item.props.ADM4_EN, e);
                    }
                });
        };
        ...
        onMounted(async () => {
            // Set view to NCR (Manila coordinates) with a tighter zoom
                map = L.map('map', { 
                    zoomControl: false, 
                    preferCanvas: true 
                }).setView([14.5995, 120.9842], 10);

                // Add a standard Leaflet Marker at the NCR center
                const ncrMarker = L.marker([14.5995, 120.9842]).addTo(map);
                ncrMarker.bindPopup('<b class="text-xs">NCR Central Hub</b><br><span class="text-[10px]">National Capital Region</span>').openPopup();

                labelLayer = L.layerGroup().addTo(map);
                // activeLayerGroup = L.featureGroup().addTo(map);
            ...
            try {
                ...
                L.geoJSON(data.features, {
                    ...
                    onEachFeature: (f, layer: any) => {
                        ...
                        layer.bindPopup(`<div class="font-bold text-xs">${f.properties.ADM4_EN}</div><div class="text-[10px]">${f.properties.ADM3_EN}</div>`);
                        }
